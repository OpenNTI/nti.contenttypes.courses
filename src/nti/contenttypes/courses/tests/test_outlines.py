...
class TestCourseOutline(CourseLayerTest):
    ...
    def test_outline_implements(self):
        assert_that(outlines.CourseOutlineNode(),
                    verifiably_provides(interfaces.ICourseOutlineNode))
        assert_that(outlines.CourseOutline(),
                    verifiably_provides(interfaces.ICourseOutline))
        assert_that(outlines.CourseOutlineNode(),
                    verifiably_provides(IRecordable))
        assert_that(outlines.CourseOutline(), verifiably_provides(IRecordable))

    def test_outline_tag_fields(self):
        fields = ('title', 'description',
                  'AvailableEnding', 'AvailableBeginning')
        for iface in (interfaces.ICourseOutlineNode,
            ...
            for name in fields:
                if name in iface:
                    value = iface[name].queryTaggedValue(
                        interfaces.TAG_HIDDEN_IN_UI)
                    assert_that(value, is_(False))
                    value = iface[name].queryTaggedValue(
                        interfaces.TAG_REQUIRED_IN_UI)
                    assert_that(value, is_(False))
    ...
    def test_outline_containment(self):
        ...
        node = outlines.CourseOutlineNode()
        outline.append(node)
        node2 = outlines.CourseOutlineContentNode(ContentNTIID=u'tag:nextthought.com,2011-10:OU-HTML-CLC3403_LawAndJustice.lec:01_LESSON',
    ...
    def test_outline_externalizes(self):
        ...
        assert_that(ext_obj, has_entries('Class', 'CourseOutlineContentNode',
                                         ...
                                         'ContentNTIID', 'tag:nextthought.com,2011-10:OU-HTML-CLC3403_LawAndJustice.lec:01_LESSON'))
