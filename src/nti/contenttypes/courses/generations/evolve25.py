...
def remove_from_container(container, key, event=False):
	# Avoid throwing intIdRemovedEvent (analytics tries to store in redis).
...
def unenroll(record, principal_id):
	try:
		...
		remove_from_container( enrollment_manager._cat_enrollment_storage, principal_id )
		remove_from_container( enrollment_manager._inst_enrollment_storage, principal_id )
